    ...
    User.findByIdAndUpdate(
      ...
      err => {
        if (err) {
          ...
          req.flash(
            "success",
            `${newExperience.name} - was successfully added to Experify! It now available to book for other Experifiers! :-)`
          );
...
router.post(
  ...
  auth.checkLoggedIn("You need to login to access this page", "/login"),
  (req, res) => {
    const idexp = req.params.id;
    // console.log('req.file', req.file);
    ...
    Experience.findOneAndUpdate(
      ...
      (err, result) => {
        ...
        req.flash(
          "success",
          `${newExperience.name} - was successfully updated! :-)`
        );
